    ...
    for _, row in roads.iterrows():
        ...
        for line in lines:
            ...
            for i in range(len(coords) - 1):
                ...
                if G.has_edge(u, v):
                    G[u][v]["weight"] = min(G[u][v]["weight"], sl)
                else:
                    G.add_edge(u, v, weight=sl)
...
def simulate_hazard(
    ...
    if intensity > 0.0:
        du  = (unary_union(gdf_s[mask].geometry.buffer(10)) if mask.any() else None)
            ...
            for kat, gt in tes_s.items():
                ...
                for idx in gt.index:
                    ...
                    if (pt and not pt.is_empty
                            and (pt.within(du) or pt.buffer(50).intersects(du))):
...
def run_pipeline(
    ...
    # ── 8. Clustering ─────────────────────────────────────────────────────────
    cl_results = run_all_clustering(gdf_sim, w, X_pca, k_opt, skenario, cfg, mask_dampak)
